...
ll _dfs(int i, ll ssf, vpll& ss, vll &a, vb &vis) {
    ...
    if(vis[i]) {
        return ss[i].second+ssf;
    }
    vis[i] = true;
    ss[i].first = ssf+a[i];
    ll tsumn = _dfs(i+a[i], ss[i].first, ss, a, vis);
    ss[i].second = (tsumn - ss[i].first);
    return tsumn;
}

void dfs(vpll& ss, vll &a, vb &vis) {
    FOR(i,1,n+1){
        ll ssum = 0;
        if(!vis[i]){
           _dfs(i, 0, ss, a, vis);
...
int main() {
    ...
    F0R(tt, t){
        ...
        FOR(i,1,n+1) cin>>a[i];
        dfs(ss, a, vis);
        sort(all(ss), [&](const auto& a, const auto&b) { return a.first > b.first; });
        ps(ss[0].first);
    }
    return 0;
}
