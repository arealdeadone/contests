...
namespace __input {
    ...
    template<class T>
    void re(vector<T> &a) { for (int i = 0; i < sz(a); i++) re(a[i]); }

    template<class T, size_t SZ>
    void re(array<T, SZ> &a) { for (int i = 0; i < SZ; i++) re(a[i]); }
}
...
namespace __output {
    ...
    struct is_outputtable {
        template<typename C>
        static constexpr decltype(declval<ostream &>() << declval<const C &>(), bool()) test(int) { return true; }
    ...
    template<class T, typename V = decltype(declval<const T &>().begin()), typename S = typename enable_if<!is_outputtable<T>::value, bool>::type>
    void pr(const T &x);

    template<class T, typename V = decltype(declval<ostream &>() << declval<const T &>())>
...
#define __pn(x) pr(#x, " = ")
#ifdef RACHURI_LOCAL
#define pd(...) pr(""), __pn((__VA_ARGS__)), ps(__VA_ARGS__), pr(""), cout << flush
#else
#define pd(...)
#endif
...
ll getSum(string s) {
    ...
    if(sz(s) == 1)
        return to_ll(s);
    ll sum = to_ll(s);
    auto getEval = [&](string str) { return 1<<(sz(str)-1); };
    for(int i=0; i< sz(s) - 1; i++) {
        const string ss = s.substr(i+1);
        const string ff = s.substr(0, i+1);
        const ll eval = getEval(ss);
        sum += eval*to_ll(ff) + getSum(ss);
    }
    return sum;
}
