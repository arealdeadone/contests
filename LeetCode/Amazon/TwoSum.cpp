...
vector<int> twoSum(vector<int>& nums, int target) {
        ...
        for(int i=0; i < sz; i++){
            ...
            auto p = lower_bound(nums.begin(), nums.end(), bal);
            bool poss = (bal != nums[i] || (positions[(p - nums.begin())].size() > 1));
            if(*p == bal && poss) {
                vector<int> ans;
                ans.push_back(positions[nums[i]].back());
                positions[nums[i]].pop_back();
                ans.push_back(positions[(p-nums.begin())].back());
                return ans;
            }
        }
        return vector<int>();
